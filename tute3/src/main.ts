...
import { concatMap, filter, map, scan, takeUntil, takeWhile } from "rxjs/operators";
...
function mousePosObservable() {
  ...
  const source$ = fromEvent<MouseEvent>(document, "mousemove");//creates observable source from fromEvent, listens for mouse move, gives MouseEvent

  source$
    .pipe(map(event => ({ x: event.clientX, y: event.clientY })),) // This must be pure
    .subscribe(pos => {elem.textContent = `${pos.x}, ${pos.y}`; 

          if (pos.x > 400) {
            elem.style.color = 'red';
          } else {
            elem.style.color = ''; // Reset to default color if x <= 400
          }
...
function animatedRect() {
  ...
  const source$ = interval(10); //10 ms

  const move$ = source$
    .pipe(
      takeUntil(interval(1000)), //terminate animation at 1 second
...
function animatedRect2() {
  ...
  const moveDownRight$ = interval(10)
    .pipe(
      ...
      takeUntil(interval(1041)),

      // Update position of rectangle

      scan((acc, curr) => {
        const newX = acc.x + 1;  // Move 1 unit right
        const newY = acc.y + 1;  // Move 1 unit down
        return { x: newX, y: newY };
      }, { x: startProps.x, y: startProps.y }), // Initial position
      
      //map(props => ({ x: props.x.toString(), y: props.y.toString() }))
    )
