...
type Todo = {
  ...
  content: ReadonlyArray<TodoItem>; // An array of TodoItem objects
};
...
const prettifyTodoItem = (item: TodoItem): string => {
  const { userId, id, title, completed } = item; //object deconstructing  
...
function prettifyBinaryTree<T extends Object>(node: BinaryTree<T>): string {
  ...
  const left = node.left ? prettifyBinaryTree(node.left).split('\n').map(line => "|- " + line).join('\n') : "";
  const right = node.right ? prettifyBinaryTree(node.right).split('\n').map(line => "|- " + line).join('\n') : "";
