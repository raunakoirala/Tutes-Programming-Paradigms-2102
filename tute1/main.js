...
const selectiveSummer = (f) => (n) => { //higher order function that takes 'f' as parameter and returned fucntion takes parameter 'n'
  const summer_aux = IMPLEMENT_THIS;//recursive
...
function map(f, list) {
  ...
  const newHead = f(head(list));
  const newRest = map(f, rest(list));
  return cons(newHead, newRest);
}
...
function filter(f, list) {
  ...
  const newHead = head(list);
  const newRest = filter(f, rest(list));
  return cons(newHead, newRest);
}
