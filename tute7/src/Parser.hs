...
char (first:rest) = Just (rest, first) -- Replace the arguments with something sensible
char _ = Nothing
...
is c s = case char s of
  ...
  Just (rest, first) | first == c -> Just (rest, first )
  _ -> Nothing
...
parseIntTuple2 s = case is '(' s of
  Just (r1, _) -> case int r1 of
    Just (r2, x) -> case is ',' r2 of
      Just (r3, _) -> case int r3 of
        Just (r4, y) -> case is ')' r4 of
          Just (r5, _) -> Just (r5, (x, y))
          _ -> Nothing
        _ -> Nothing
      _ -> Nothing
    _ -> Nothing
...
leaf (first:rest) | first == 'L' = Just (rest, Leaf)
leaf _ = Nothing
...
node s = case is '(' s of
  Just (r1, _) -> case int r1 of
    Just (r2, x) -> case parseBinaryTree r2 of
      Just (r3, left) -> case parseBinaryTree r3 of
        Just (r4, right) -> case is ')' r4 of
          Just (r5, _) -> Just (r5, Node x left right)
          _ -> Nothing
        _ -> Nothing
      _ -> Nothing
    _ -> Nothing
